...
function printHelp () {
  ...
  echo
  echo "	eyfn.sh generate -c mazenchannel"
  echo "	eyfn.sh up -c mazenchannel -s couchdb"
  echo "	eyfn.sh up -l node"
  echo "	eyfn.sh down -c mazenchannel"
  echo
...
function networkUp () {
  ...
  if [ ! -d "dell-artifacts/crypto-config" ]; then
    generateCerts
  ...
  echo "###############################################################"
  docker exec Org3cli ./scripts/step2dell.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo "###############################################################"
  docker exec cli ./scripts/step3dell.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to add Org3 peers on network"
    exit 1
  fi
  # finish by running the test
  docker exec Org3cli ./scripts/testdell.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function createConfigTx () {
  ...
  echo "###############################################################"
  docker exec cli scripts/step1dell.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function generateCerts (){
  ...
  (cd dell-artifacts
   set -x
   cryptogen generate --config=./dell-crypto.yaml
   res=$?
   set +x
   if [ $res -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  (cd dell-artifacts
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg dellMSP > ../channel-artifacts/dell.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
  ...
  cp -r crypto-config/ordererOrganizations dell-artifacts/crypto-config/
  echo
}
...
# use this as the default docker-compose yaml definition
COMPOSE_FILE_ORG3=docker-compose-dell.yaml
#
COMPOSE_FILE_COUCH_ORG3=docker-compose-couch-dell.yaml
