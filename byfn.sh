...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"mazenchannel\")"
  ...
  echo
  echo "	byfn.sh generate -c mazenchannel"
  echo "	byfn.sh up -c mazenchannel -s couchdb"
  echo "        byfn.sh up -c mazenchannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c mazenchannel"
  echo "        byfn.sh upgrade -c mazenchannel"
  echo
...
function networkUp() {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    ...
    if [ "$CONSENSUS_TYPE" == "kafka" ]; then
      ...
      IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_CA up -d 2>&1
      docker ps -a
    else
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.mazen.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.mazen.com
    docker cp -a orderer.mazen.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.mazen.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.mazen.com

    for PEER in peer0.microsoft.mazen.com peer1.microsoft.mazen.com peer0.oracle.mazen.com peer1.oracle.mazen.com peer0.dell.mazen.com peer1.dell.mazen.com; do
...
function networkDown() {
  # stop dell containers also in addition to microsoft and oracle, in case we were running sample to add dell
  # stop kafka and zookeeper containers in case we're running with kafka consensus-type
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_dell -f $COMPOSE_FILE_CA down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./dell-artifacts/crypto-config/ channel-artifacts/dell.json
    # remove the docker-compose yaml file that was customized to the mazen
...
# and two Peer Orgs (``microsoft`` & ``oracle``) each managing and maintaining two peer nodes.
...
# (``peer0.microsoft.mazen.com`` & ``peer0.oracle.mazen.com``).  Secondly, we point to
...
CLI_DELAY=3
# channel name defaults to "mazenchannel"
...
# dell docker compose file
COMPOSE_FILE_dell=docker-compose-dell.yaml
